...
> A multi-agent AI system that automates NFT minting, liquidity pool management, on-chain intelligence, and security auditing on Solana and EVM chains — powered by Hermes Agent.
...
- AI-driven strategy learning from top LPers
...
- Rug pull detection with 10+ red flag patterns
...
```
1. POOL DISCOVERY
   → Screen DLMM pools via Meteora API
   → Score: fee/TVL ratio, volume, liquidity depth
   → Filter: pools with >$20K TVL, >5% fee/TVL

2. STRATEGY LEARNING
   → Query Agent Meridian API for top LPers per pool
   → Analyze: strategy type (spot/bid-ask), bin range, hold time
   → Calculate: win rate, avg PnL per strategy
...
2. **LP Strategy Research**: Analyzed top LPers via Meridian API
3. **Pool Screening**: Scored DLMM pools in real-time
4. **Security Audit**: Scanned ClawHub skills for vulnerabilities
...
# Run agents
node agents/lp/lp_bot.js auto     # LP management (auto mode)
python3 agents/mint/mint_bot.py   # NFT mint agent
python3 agents/security/rug_detector.py <TOKEN>  # Rug pull scan
```
...
│   │   └── README.md
│   ├── intel/          # On-chain intelligence (planned)
│   │   └── README.md
...
- **Skill auditing**: Skills scanned before installation
